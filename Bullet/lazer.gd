...
func _process(_delta: float):
	...
	for i:RayCast2D in ray_array:
		...
		if i.is_colliding():
			if (i.get_collider()!=null and i.get_collider().get_parent()!=null and i.get_collider().get_parent() in get_tree().get_nodes_in_group("Units")\
					or is_instance_valid(i.get_collider()) and i.get_collider().is_in_group("Buildings")) and damaging==true:
